...
public:
	...
	BOOL LoadCOLORREFimage(HWND hWnd);
	BOOL Repaint(void);
	...
	void DeleteColorImage(void);
	COLORREF* GetColorImage(void);
	COLORREF* CreateColorImage(int xsize, int ysize, int Numframes);
	int  LoadBMPfile(WCHAR* InputFilename, BOOL Invert);
};